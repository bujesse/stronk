...
function nullableText<T extends string | null | undefined>(value: T | ''): T | null {
...
const syncEntityConfigs = {
  exercise: {
    ...
    deserialize: (row: RemoteExerciseRow): Exercise => ({
      ...
      muscleGroup: nullableText(row.muscle_group),
      equipment: nullableText(row.equipment),
      preferredWeightUnit: nullableText(row.preferred_weight_unit),
  ...
  loggedSet: {
    ...
    deserialize: (row: RemoteLoggedSetRow): LoggedSet => ({
      ...
      plannedSetId: nullableText(row.planned_set_id),
      sortOrder: row.sort_order,
      setKind: row.set_kind,
      reps: row.reps,
      weight: row.weight,
